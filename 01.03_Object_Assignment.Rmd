...
From the picture above, you can see that the value of `x_num` is 5. Let's try to assign the value 6 to x_num and see what will happen next.
...
Now you can see that the value of `x_num` has changed from 5 to 6. 所以如果assign different values to the same name, object的value会由最新一次的assignment operation决定，之前的value会被覆盖。Environment 中会显示the object和更新后的value。
