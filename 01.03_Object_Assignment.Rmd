...
Sometimes, you may build an object based on another objects. Let's say we assign the object `a` and object `b` to the **value** 3 and 4, respectively. The object `c` is assigned to be the sum of `a` and `b`. Consequently, we should expect `c` to be the sum of 3 and 4, which is 7.
...
Now, you want to assign `a` new value to a from 3 to 7. However, if you run `c` again right after reassigning `a`, you can see the output is still 7. Indeed, an object won't be auto-updated when you reassign any objects it contains. To update the object `c` with the newly assigned `a`, you need to re-run the assignment operation, which will then give you 7 + 4, namely 11, as the value of `c`.
