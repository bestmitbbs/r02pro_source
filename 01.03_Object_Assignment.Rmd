...
Sometimes, you may build an object based on another objects. Let's say we assign the object `a` and object `b` to the value **value** 3 and 4, respectively. The object `c` is assigned to be the sum of `a` and `b`. Consequently, we should expect `c` to be the sum of 3 and 4, which is 7.
...
Now, you want to assign `a` new value to a from 3 to 7. However, if you run `c` again right after reassigning `a`, you can see the output is still 7. Indeed, an object won't be auto-updated when you reassign any objects it contains. In other words, to update the object `c` with the newly assigned `a`, you should re-run the assignment operation, which will then give you 7 + 4, namely 11, as the value of `c`.
...
At this point, we've introduced the way to and rules of creating objects in R. Unless an error message is returned, we know an object is successfully created. Now, we can also confirm the success of object assignment by inspecting the **Environment**, located in the top right panel (**panel3 in Figure \@ref(fig:four)**). 

If you exercised previous examples, you can find the newly assigned objects `x_num` and `y_num` (possibly also `z_num1` and `z_num2`) in your **Environment** Viewer. You may also notice that the object 'TRUE,' which we tried but failed to assign the value 12 to, doesn't appear in the **Environment**. You can check all the **named objects** and their values in this area. It is helpful to monitor the environment from time to time to make sure everything looks fine. Notice that objects without names will *not* be shown in the environment.

You can also see the list of all the named objects using the built-in R function `ls()`.
