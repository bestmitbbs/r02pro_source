...
So far, we have been working with the ideal case that the vector is fully observed. In most applications, you may encounter the situation where some values are **missing** or **undefined** in the data set for certain variables, and such values can be resulted from one of the below situations:

a) the observer fails to record the data, meaning the value is not available
b) the data is impossible or undefined, meaning the value cannot be generated, such as dividing by 0.

In two scenarios, R uses a) `NA`, indicating they are **not available**, and b) `NaH`, indicating they are **not a number** to present those values, respectively. Let's see the following example. 
...
If you look at the environment panel, you will see that `b` has a value of `NaH`, because arithmetically you cannot divide a number by 0.
...
By running the above codes, you can notice that `NA` is stored as a logical type, whereas `NaH` is stored as a numeric type.

Now you have had a basic understanding of `NA` and `NaH`. In the following parts of this section, we want to introduce several properties of them. Let's start with the fact that `NA` is **contagious**. 

### `NA` and `NaH` are contagious
...
Similar to `NA`, `NaH` is also contagious, because numeric calculations on undefined values are also undefined.
...
Moreover, if a vector consists undefined values, applying statistical functions on it will also result in `NaH`, and you can still use `na.rm` to remove `NaH` before applying the functions.
