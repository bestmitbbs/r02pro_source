...
Knowing how to export data into delimited files, let's see how to **import** data from delimited files.
...
We can see that there is a message showing the **Column specification** during the import process. In particular, we see `country` is of type *character*, and both `year` and `HDI` are both of type *double*. We can also check the value of `gm_small` and its structure.
