...
Having learned data frames in Section \@ref(dataframe), we would like to introduce a modern version of data frame, named **tibbles**. Tibbles **are** data frames, but change some behaviors of data frames to make coding easier. To use the tibble class, you need to install the **tibble** package, which is part of the **tidyverse** package. 
...
After installing the **tibble** package, you can load the package and create a tibble using the `tibble()` function using all variables as its argument, which is similar to how you create a data frame.
```{r, results=TRUE}
library(tibble)
