...
In this section, you will learn about **numeric vectors**. Numeric vector is perhaps the most commonly used member of the **atomic vector** family, where all elements are of the same type. 
...
A **numeric vector** is a type of atomic vector that only contains numbers. For example, `6` is a numeric vector with one element of value 6. 
 

After assigning the value 6 to the name x1, you have created a new numeric vector `x1`. And you can refer to `x1` in the subsequent calculations. For any vector, you can use the `length()` function to check the number of elements it contains.
...
You have successfully created a numeric vector `x1` of length 1. But can a numeric vector contain more than one values? The answer is a big YES! In R, you can use the `c()` function (`c` is short for combine) to combine elements into a numeric vector. 
...
In addition to using numbers inside the `c()` function, you can also use several numeric vectors as the arguments, which creates a longer vector, as a combination of the input numeric vectors in the given order. 
...
For any R object, you can use the function `class()` to check its **class**. The class of an object provides a description about the object, and determines what functions can be applied to it. 
...
To **access an element** from a vector, you can use the vector name followed by the index of the desired element surrounded by a pair of `[` and `]`. Let's see some examples.
...
Let's look at another example where we want to modify the third element of `y2` to be the fourth element of `y1` times 2.
```{r, results = TRUE}
...
Since numeric vectors are made of numbers, you can do **arithmetic operations** between them, just like the fancy calculator in Section \@ref(Calculator). If two vectors are of the **same length**, the operation is done **element-wisely**. In other words, R will perform the operation separately for each element.  First, let's create another vectors `x2` of length 1, and compute the sum of `x1` and `x2`.
...
You have already learned to type the object name to reveal its value. Let's try to type `pi`, which is an internal constant available for use.
```{r, results=TRUE}
pi
```

As you can see from the output, R prints out 7 significant digits by default, though in fact we need infinitely many digits to faithfully represent `pi`. To customize the number of significant digits, you can use the `print()` function.  In the `print()` function, there is a useful argument called `digits`, which controls the minimal number of significant digits. Let's see the following examples.
```{r, results = TRUE}
print(pi)
...
2. For numeric vector `vec_2 <- c(1, 3, 7, 10)`, get the value of the 3rd element, multiple the 3rd element by 5, and verify the change. 
...
5. Print the vector $(e, e^2, e^3)$ with 5 significant digits. 
