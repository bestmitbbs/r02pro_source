## Bar Charts via geom_bar() {#bar-charts}
...
```{r}
ggplot(data = sahp) + 
  geom_bar(mapping = aes(x = kit_qual, y = stat(prop), group = 1))
```
...
library(forcats)
ggplot(data = sahp) + 
  geom_bar(mapping = aes(x = fct_reorder(kit_qual, kit_qual, length))) #increasing order
```
...
```{r}
ggplot(data = sahp) + 
  geom_bar(mapping = aes(x = fct_reorder(kit_qual, kit_qual, length, .desc = TRUE))) #decreasing order
```
...
```{r}
ggplot(data = sahp) +
  geom_bar(mapping = aes(x = fct_relevel(kit_qual, c("Average", "Fair", "Good", "Excellent"))))
```
...
```{r, eval = FALSE}
ggplot(data = sahp) + 
  geom_bar(mapping = aes(x = kit_qual, fill = central_air), position = "stack")
```
...
```{r}
ggplot(data = sahp) + 
  geom_bar(mapping = aes(x = kit_qual, fill = central_air), position = "dodge")
```
...
```{r}
ggplot(data = sahp) + 
  geom_bar(mapping = aes(x = kit_qual, fill = central_air), position = "fill")
```
