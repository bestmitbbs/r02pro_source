...
So far, we have learned atomic vectors (Chapter \@ref(r-objects)), matrices (Section \@ref(matrix)), and arrays (Section \@ref(array)). The three different object types share an important features: they all contain elements of the same type, for example numeric, character, or logical. In real applications, it is common to have mixed variable types. To accommodate this, let's introduce a new object type, namely the **data frame**. 
...
In Section \@ref(numeric-vector-statistics), we introduced the very useful function `summary()` which gives us important summary statistics for a vector. Using `summary()` on a data frame, you get the summary statistics for each variable. 
