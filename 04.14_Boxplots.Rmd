...
ggplot(data = remove_missing(sahp, vars = "sale_price")) + 
  geom_boxplot(aes(x = fct_reorder(kit_qual, sale_price, median), y = sale_price)) 
```

To order it by the mean `sale_price` in descending order, you can use 
```{r}
ggplot(data = remove_missing(sahp, vars = "sale_price")) + 
  geom_boxplot(aes(x = fct_reorder(kit_qual, sale_price, mean, .desc = TRUE), y = sale_price)) 
```
...
g <- ggplot(data = na.omit(sahp), aes(x = house_style, y = sale_price)) +
  scale_x_discrete(limits=c("1Story", "2Story"))
g + geom_boxplot()
```
...
```{r, warnings = FALSE}
ggplot(data = sahp) + 
  geom_boxplot(aes(x = "", y = sale_price), notch = TRUE)
```
