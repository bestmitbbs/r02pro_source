...
If the first argument is of length larger than 1, and the second argument is a single integer, the `rep()` function will **repeat** the whole vector the corresponding number of times. 

```{r}
rep(c("sheep", "monkey"), 3)
```

Another option is the specify the second element as a numeric vector of the same length as the vector to be repeated. In this case, each value of the character vector will be sequentially repeated the corresponding number of times. Please see the following examples. 
...
In `paste()`, the default **separator** between the individual strings is space. In fact, you can change the separator by setting the `sep` argument as a string in `paste()`. For example, you can separate the individual strings with comma or `!&!`.
...
If you would like to concatenate the elements of a vector into a longer string, you need to specify the `collapse` argument as the separator instead of `sep` in the `paste()` function. 
...
### Exercises
1. Write R codes using the  `rep()` function to reproduce the following three vectors.
```{r, echo = FALSE, results = TRUE}
rep(letters[1:4], 3)
rep(letters[1:4], rep(3, 4))
rep(letters[1:4], 4:1)
```

2. Please use the `paste()` function to reproduce the following sentences.
```{r, echo = FALSE, results = TRUE}
names <- c("Alice", "Bob", "Charlie")
sports <- c("tennis", "soccer", "baseball")
years <- c(4:2)
locations <- c("London", "New York", "Berlin")
paste(names, "has been playing", sports, "for", 
      years, "years in", locations)
```

3. Please use the `paste()` and `cat()` functions to reproduce the following message. 
```{r, echo = FALSE, results = TRUE}
